...
public class FindEvent {
	public static void sendFindList(Player p, String search, int page){
		int eventnum = 0;
		for(String item : StoreEvent.eventlist.keySet()){
			boolean found = false;
			String[] clist = StoreEvent.eventlist.get(item).split(" ");
		...
		if(eventnum == 0){
			p.sendMessage(ChatColor.RED + "There are currently no active events that contain that keyord in its description.");
		}
		...
		if(page <= pagenum && page > 0){
			...
			if(eventnum > 0){
				...
				p.sendMessage(ChatColor.GOLD + "List of events containing the keyword " + search);
				...
				for(String entry : StoreEvent.eventlist.keySet()){
					String[] list = StoreEvent.eventlist.get(entry).split(" ");
					boolean found = false;
